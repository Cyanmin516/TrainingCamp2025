...
## 安裝
0. 檢測網路線長度、檢測網路水晶街頭好壞(腳針及溝槽)，確定伺服器插線對應的網路孔與顯示孔。
1. 拆解硬體: 先拆後裝，拆卸遠到近，組裝近到遠
2. 不熟悉的分階段安裝，最低需求成功後才組裝後面零件。
   - 主機板+電源+CPU
   - +NIC+GPU+RAM
   - +SATA+光碟機
   - 正常開機，檢查BIOS，虛擬化、SATA裝置
   - 網路線，製作跳線A_to_B，A白綠綠白橙藍白藍橙白棕棕，B白橙橙白綠藍白藍綠白棕棕，左手抓線要穩
   - 開通ICMPv4規則或關閉防火牆

3. 製作USB開機安裝
    - rufus 4.7 : ISO-windows-server-2019, GPT-UEFI, no user.
    - copy 資料: client ISO win10_22, vm, srever2019, fedora
    - 先安裝server
    - 再安裝client
...
- 172.16.140.100/24
    3.1用跳接線連接client
    - 製作網路接頭 568A 白綠綠白橙藍白藍橙白棕棕; 568B 白橙橙白綠藍白藍綠白棕棕
    - 檢測(13,26,31,44,55,62,77,88)
4. firewall
- close or wf.msc,allow ICMP ipv4 private
- disable
5. members
- 本機安全性原則, 密碼原則, 複查度, 停用
- master(+group:administrators)
- user1
- user2
6. NTFS
管理資料夾權限給使用者
...
17. FTP 無法使用網址登入但可以用IP登入
- 重新安裝FTP服務
18. www網域失敗但是localhost可以顯示
- 實體網路線沒有通訊
19. 網卡消失
- 顯示卡熱風導致熱當機
20. ftp無法登入
...
**Fedora**
- 安裝時設定bridge
- 啟動直接跳過安裝，terminal ping, traceroute
